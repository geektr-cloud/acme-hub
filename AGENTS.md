...
除 pull 触发（消费者调 `/pki/v1/certificates/issue` 时按 `decide()` 顺带续期）外，还有主动定时续期。Cron Trigger（`wrangler.jsonc` `triggers.crons`，`0 */3 * * *`，每 3 小时，对齐 Let's Encrypt 3h 速率窗口）→ `server/index.ts` 的 `scheduled` handler `ctx.waitUntil(scanAndRenew())`。纯 D1 + `ctx.waitUntil`，无 Queue/DO/Workflow。

- **预约窗口调度**（`server/core/pki/schedule.ts`，纯函数、有单测）：负载均衡在**签发时**做，非 cron 事后现算。每天切 8 个 3h 窗口（UTC 起点 `00:00/03:00/…/21:00`，全 UTC）。`windowStart(t)` floor 归一；`pickSlot(理论renewAt, load)` 从理论时点所在窗口**倒序**（往早）遍历前 7 天（`LOOKBACK_WINDOWS=56`）挑负载最小窗口，平局取最晚（最靠近理论时点）。只向早移 → 续得更早、离过期更远。
- **唯一写入点**：`certificates.renewAt`（窗口对齐 ISO 时间戳，nullable，有 `Certificate_renewAt_idx` 索引）**只在 `runIssue` 落库那步计算 + 写入**（`loadWindowOccupancy` 查窗口占用 → `pickSlot`）。pull 与 cron 共用此路径 → 两条路自动一致；pull 提前续期后 `renewAt` 自动刷新到未来窗口。**严禁在别处重算 `renewAt`**（否则续过的行窗口停在过去，cron 反复重挑）。
- **`scanAndRenew`**（`server/core/pki/cron.ts`）：只查 `renewAt IS NOT NULL AND renewAt <= now AND certificate != '' AND acmeAccountId != NULL`，按 `renewAt` 升序取前 `MAX_RENEW_PER_RUN`（10）张，逐张 `try/catch` 隔离续期。`renewAt IS NULL` 的行忽略（正常流程新签发都经唯一写入点带上 `renewAt`，无回填逻辑）。
- **并发安全**：cron 续期复用 pull 的 `ResourceLock`——每张先 `acquire` 域名锁，拿不到（正被 pull 或另一 cron 占用）即跳过、不等待。调 `runIssue` 省略 `emit`（`CertEvent` 仍写，无实时推流）。
- **限量结转**：单轮超 `MAX_RENEW_PER_RUN` 的行 `renewAt` 仍在过去，下轮自然接续。这是超载优雅降级阀，勿绕过。承载：串行约 60s/张、10 张/轮，实际舒适 1000-2000 张；超此规模再演进（预约调度层不变，只换执行出口）。
- **本地触发 cron**：`@cloudflare/vite-plugin` 下 `/__scheduled` 会命中 SPA fallback**不触发** scheduled，须用 `curl "http://localhost:<port>/cdn-cgi/handler/scheduled?cron=0+*/3+*+*+*"`。
