...
`POST /api/acme/v1/cert`，body `{ domains: string[] }`，响应 `{ domain, key, cer, ca, notAfter }`（`domain` 为 primary = 数组首项，签发证书 SAN 包含全部 domains）。外部消费方（Caddy 等）单端点拉证书，签发/续期/缓存由服务端透明决策。
...
- **决策**：查同 account+primary domain 最新证书。剩余寿命比例（`config.ts` `RENEW_RATIO = 0.5`）内直接回缓存；过半/过期/解析失败则续期（复用旧 key+csr）；无记录则新签。缓存/续期前额外比对 SAN 集合：请求域名不是旧证书 alt 子集时强制新签（`san.ts` `needNewKeypair`）。
