...
业务实体：`acme-accounts`（ACME 账户）、`certificates`（证书）、`clients`（客户端）。
...
`server/index.ts` 导出 Hono `app`（+ `AppType` 供前端 RPC）。在 `/api/{auth,acme-accounts,clients,certificates}` 挂载各路由模块，最后 `app.get("/*", ...)` 用 `env.ASSETS` 提供 SPA。错误经 `ErrorHandler`（来自 `@acrux/server`）序列化为 `{ status, error }`。

### Auth gate

所有 `/api/*`（除 `/api/auth/*`）走签名 cookie 中间件 `server/middlewares/auth.ts`。登录 `POST /api/auth/login` 用 `env.API_TOKEN` HMAC 签发 HttpOnly cookie。前端 `rpc()` 拦截 401 → 跳 `/login?next=`。

**新建对外暴露的 endpoint 时，确认它在 auth 中间件之后；若有意做成公开接口，必须显式说明并确认。**
